...
const getPdfStyles = (formatting: any) => {
  ...
  return StyleSheet.create({
    page: { padding: 30, fontFamily: pdfFont, fontSize: 9, lineHeight: 1.3, color: currentColors.text },
    headerName: { fontSize: 18, fontWeight: "bold", textAlign: "center", textTransform: "uppercase", color: currentColors.primary },
    headerSub: { fontSize: 8, textAlign: "center", color: "#555555", marginTop: 4 },
    sectionTitle: { fontSize: 10, fontWeight: "bold", borderBottomWidth: 1, borderBottomColor: currentColors.primary + "33", marginTop: 14, paddingBottom: 2, textTransform: "uppercase", color: currentColors.primary },
    entryTitleRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginTop: 6 },
    entryTitle: { fontSize: 9, fontWeight: "bold", color: "#111111", flex: 1 },
    entryDuration: { fontSize: 9, color: "#666666", marginLeft: 10, flexShrink: 0 },
    summaryText: { marginTop: 6, fontSize: 9, fontStyle: "italic", color: "#444444" },
    bulletList: { marginTop: 4, paddingLeft: 12 },
    bulletItem: { flexDirection: "row", alignItems: "flex-start", marginBottom: 2 },
    bulletPoint: { width: 8, fontSize: 9, color: currentColors.primary },
    bulletText: { flex: 1, fontSize: 9 },
    skillsText: { marginTop: 6, fontSize: 9, color: "#333333" }
  });
};
...
export async function POST(request: NextRequest) {
  try {
    ...
    const userEmail = request.headers.get("x-user-email") || "thirumuruganaks@gmail.com";
    ...
    await sendEmail({
      to: userEmail,
      subject: `Your Exported Resume: ${resume.title}`,
      text: `Hello,\n\nPlease find attached your exported resume "${resume.title}" in PDF format.\n\nBest regards,\nResume Buddy AI`,
      html: `<p>Hello,</p><p>Please find attached your exported resume <strong>"${resume.title}"</strong> in PDF format.</p><p>Best regards,<br/>Resume Buddy AI</p>`,
      attachments: [
        {
    ...
    return NextResponse.json({ success: true, message: `Resume emailed successfully to ${userEmail}` });
  } catch (error: any) {
    console.error("Email export error:", error);
    return NextResponse.json({ error: error.message || "Failed to email resume" }, { status: 500 });
  }
}
